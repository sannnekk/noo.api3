        ...
        INotificationHandler<AssignedWorkSolveDeadlineShiftedEvent>
{
    ...
    public AssignedWorkNotificationEventHandlers(
        INotificationService notificationService,
        IAssignedWorkRepository assignedWorkRepository,
        IUserRepository userRepository,
        IAssignedWorkLinkGenerator assignedWorkLinkGenerator
    )
    {
    ...
    public async Task Handle(
        HelperMentorRemovedEvent notification,
    ...
    public async Task Handle(
        AssignedWorkCheckedEvent notification,
        CancellationToken cancellationToken
    )
    {
        var assignedWork = await _assignedWorkRepository.GetByIdAsync(notification.AssignedWorkId);
        ...
        var ToSendIds = new List<Ulid?>()
        {
            ...
            assignedWork.HelperMentorId.HasValue ? assignedWork.HelperMentorId.Value : Ulid.Empty,
        }
        ...
        await _notificationService.BulkCreateNotificationsAsync(
            new()
            {
                ...
                Message =
                    $"Работа \"{assignedWork.Title}\" была проверена, балл: {assignedWork.Score}/{assignedWork.MaxScore}.",
    ...
    public async Task Handle(
        AssignedWorkCheckDeadlineShiftedEvent notification,
        CancellationToken cancellationToken
    )
    {
        var assignedWork = await _assignedWorkRepository.GetByIdAsync(notification.AssignedWorkId);
        ...
        await _notificationService.BulkCreateNotificationsAsync(
            new()
            {
                ...
                Message =
                    $"{user.Name} сдвинул(а) дедлайн проверки работы \"{assignedWork.Title}\" на {newDeadline}.",
            }
        );
    }

    public async Task Handle(
        AssignedWorkSolveDeadlineShiftedEvent notification,
        CancellationToken cancellationToken
    )
    {
        var assignedWork = await _assignedWorkRepository.GetWithStudentAsync(
            notification.AssignedWorkId
        );
        ...
        await _notificationService.BulkCreateNotificationsAsync(
            new()
            {
                ...
                Message =
                    $"{assignedWork.Student.Name} сдвинул(а) дедлайн решения работы \"{assignedWork.Title}\" на {newDeadline}.",
            }
        );
    }

    public async Task Handle(
        MainMentorChangedEvent notification,
        CancellationToken cancellationToken
    )
    {
        var assignedWork = await _assignedWorkRepository.GetByIdAsync(notification.AssignedWorkId);
        ...
        if (!assignedWork.MainMentorId.HasValue)
        {
            return;
        }

        var user = await _userRepository.GetByIdAsync(assignedWork.MainMentorId.Value);
        ...
        await _notificationService.BulkCreateNotificationsAsync(
            new()
            {
                ...
                Message =
                    $"Теперь главным куратором работы \"{assignedWork.Title}\" является {user.Name}",
            }
        );
    }

    public async Task Handle(
        AssignedWorkSolvedEvent notification,
        CancellationToken cancellationToken
    )
    {
        var assignedWork = await _assignedWorkRepository.GetByIdAsync(notification.AssignedWorkId);
    ...
    public async Task Handle(
        HelperMentorAddedEvent notification,
        CancellationToken cancellationToken
    )
    {
        var assignedWork = await _assignedWorkRepository.GetByIdAsync(notification.AssignedWorkId);
