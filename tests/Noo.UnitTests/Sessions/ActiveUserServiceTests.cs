...
{
    private static (ActiveUserService svc, Mock<ICacheRepository> cache, SessionConfig config) Create()
    {
        var cache = new Mock<ICacheRepository>(MockBehavior.Strict);
        var config = new SessionConfig();
        var options = Options.Create(config);
        var svc = new ActiveUserService(cache.Object, options);
    ...
    {
        var (svc, cache, config) = Create();
        var userId = Ulid.NewUlid();
        const UserRoles role = UserRoles.Admin;

        cache.Setup(c => c.SetAsync<DateTime>(It.Is<string>(k => k.StartsWith("active:user:")), It.IsAny<DateTime>(), config.ActiveTtl))
            .Returns(Task.CompletedTask);
        cache.Setup(c => c.SetAsync<DateTime>(It.Is<string>(k => k.StartsWith("active:role:")), It.IsAny<DateTime>(), config.ActiveTtl))
            .Returns(Task.CompletedTask);
    ...
    public async Task SetUserActive_OnlyUserKey_WhenRoleInvalidEnumValue()
    {
        ...
        cache.Verify(c => c.SetAsync<DateTime>(It.Is<string>(k => k.StartsWith("active:role:")), It.IsAny<DateTime>(), config.ActiveTtl), Times.Never);
    }
    ...
    public async Task IsUserActive_ReturnsFalse_WhenLastOutsideTtl()
    {
        var (svc, cache, config) = Create();
        var userId = Ulid.NewUlid();
        var past = DateTime.UtcNow - config.ActiveTtl - TimeSpan.FromMinutes(1);
